...
#ifndef CERTAINITY_USE_MMAP
#    ifdef __linux__
#        define CERTAINITY_USE_MMAP 1
#    else
#        define CERTAINITY_USE_MMAP 0
#    endif
#endif

#if CERTAINITY_USE_MMAP
#    include <fcntl.h>
#    include <sys/mman.h>
#    include <sys/stat.h>
#    include <unistd.h>
...
#    define CERTAINITY_STRLEN(str)  __builtin_strlen(str)
#    define CERTAINITY_STRCMP(a, b) __builtin_strcmp(a, b)
#else
constexpr inline std::size_t CERTAINITY_STRLEN(const char* str) {
    std::size_t len = 0;
    while (*str++) ++len;
    return len;
}

constexpr inline int CERTAINITY_STRCMP(const char* a, const char* b) {
    while (*a && *b && *a == *b) {
        ++a;
        ++b;
    }
...
#    define CERTAINITY_STRLEN(str)  ::command_line_options::detail::CERTAINITY_STRLEN(str)
#    define CERTAINITY_STRCMP(a, b) ::command_line_options::detail::CERTAINITY_STRCMP(a, b)
#endif

/// Raise a compile-time error.
#ifndef CERTAINITY_ERR
#    define CERTAINITY_ERR(msg) [] <bool _x = false> { static_assert(_x, msg); } ()
#endif
...
struct static_string {
    ...
    [[nodiscard]] constexpr bool operator==(const str& s) const {
        return len == s.len && CERTAINITY_STRCMP(arr, s.arr) == 0;
    }
    ...
    constexpr void append(const char* str) { append(str, CERTAINITY_STRLEN(str)); }
...
static file_data_type map_file(
    ...
    else CERTAINITY_ERR("file_data_type::contents_type must have a callable assign member that takes a pointer and a size_t (or a begin and end iterator) as arguments.");
    ::munmap(mem, sz);
...
static consteval auto type_name() -> static_string<25> {
    ...
    if constexpr (detail::is<t, std::string>) buffer.append("string");
        ...
        CERTAINITY_ERR("Option type must be std::string, bool, integer, double, or void(*)(), or a vector thereof");
    }
    return buffer;
}
...
class CERTAINITY_impl;

template <typename... opts, typename... special>
class CERTAINITY_impl<list<opts...>, list<special...>> {
    // This should never be instantiated by the user.
    explicit CERTAINITY_impl() = default;
    ~CERTAINITY_impl() = default;
    CERTAINITY_impl(const CERTAINITY_impl& o) = delete;
    CERTAINITY_impl(CERTAINITY_impl&& o) = delete;
    CERTAINITY_impl& operator=(const CERTAINITY_impl& o) = delete;
    CERTAINITY_impl& operator=(CERTAINITY_impl&& o) = delete;
    ...
    static constexpr size_t optindex_impl() {
        ...
        else if constexpr (CERTAINITY_STRCMP(opt_names[index], option.arr) == 0) return index;
...
public:
    ...
    class optvals_type {
        ...
        constexpr auto get_impl() -> get_return_type<opt_by_name<s>> {
            ...
            else if constexpr (detail::is_callback<canonical>) CERTAINITY_ERR("Cannot call get<>() on an option with function type.");
...
private:
    ...
    [[nodiscard]] constexpr auto ref_to_storage() -> decltype(std::get<optindex<s>()>(optvals.optvals))& {
        ...
        if constexpr (std::is_same_v<value_type, bool>) CERTAINITY_ERR("Cannot call ref() on an option<bool>");

        // Function options don’t have a value.
        else if constexpr (detail::is_callback<value_type>) CERTAINITY_ERR("Cannot call ref<>() on an option with function type.");
...
private:
    ...
    auto make_arg(std::string_view opt_val) -> typename opt::single_element_type {
        ...
        if constexpr (not detail::has_argument<element>) CERTAINITY_ERR("This option type does not take an argument");
        ...
        else CERTAINITY_ERR("Unreachable");
    }
...
template <typename... opts>
using CERTAINITY = detail::CERTAINITY_impl< // clang-format off
...
#undef CERTAINITY_STRLEN
#undef CERTAINITY_STRCMP
#undef CERTAINITY_ERR
#endif // CERTAINITY_H
