...
using basic_options = CERTAINITY<
...
TEST_CASE("Required options must be present") {
    using options = CERTAINITY<option<"--required", "A required option", std::string, true>>;
    CHECK_THROWS(options::parse(0, nullptr, error_handler));
}

TEST_CASE("Flags are never required") {
    using options = CERTAINITY<flag<"--flag", "A flag">>;
...
TEST_CASE("values<> option type is handled properly") {
    using int_options = CERTAINITY<option<"--values", "A values option", values<0, 1, 2, 3>>>;
    using string_options = CERTAINITY<option<"--values", "A values option", values<"foo", "bar", "baz">>>;
...
TEST_CASE("Positional options are required by default") {
    using options = CERTAINITY<positional<"first", "The first positional argument">>;
    CHECK_THROWS(options::parse(0, nullptr, error_handler));
}

TEST_CASE("Positional values<> work") {
    using string_options = CERTAINITY<positional<"format", "Output format", values<"foo", "bar">>>;
    using int_options = CERTAINITY<positional<"format", "Output format", values<0, 1>>>;
...
TEST_CASE("Multiple positional values<> work") {
    using string_options = CERTAINITY<multiple<positional<"format", "Output format", values<"foo", "bar">>>>;
    using int_options = CERTAINITY<multiple<positional<"format", "Output format", values<0, 1>>>>;
...
TEST_CASE("Empty option value is handled correctly") {
    ...
    SECTION("for strings") {
        using options = CERTAINITY<option<"--empty", "An empty string", std::string>>;
        auto opts = options::parse(args.size(), args.data(), error_handler);
    ...
    SECTION("for integers") {
        using options = CERTAINITY<option<"--empty", "An empty integer", int64_t>>;
        CHECK_THROWS(options::parse(args.size(), args.data(), error_handler));
    }

    SECTION("for floats") {
        using options = CERTAINITY<option<"--empty", "An empty float", double>>;
        CHECK_THROWS(options::parse(args.size(), args.data(), error_handler));
    }

    SECTION("for values<> that contains the empty string") {
        using options = CERTAINITY<option<"--empty", "An empty value", values<"">>>;
        auto opts = options::parse(args.size(), args.data(), error_handler);
        REQUIRE(opts.get<"--empty">());
        CHECK(opts.get<"--empty">()->empty());
    }
...
TEST_CASE("Integer overflow is an error") {
    using options = CERTAINITY<option<"--overflow", "A number", int64_t>>;
...
TEST_CASE("Calling from main() works as expected") {
    using options = CERTAINITY<option<"--number", "A number", int64_t>>;
...
TEST_CASE("File option can map a file properly") {
    auto run = []<typename file> {
        using options = CERTAINITY<option<"file", "A file", file>>;
...
TEST_CASE("Overridable options work") {
    ...
    using options1 = CERTAINITY<option<"-x", "A string", std::string, false, true>>;
    using options2 = CERTAINITY<overridable<"-x", "A string">>;
